...
@RestController public class GitHubClient {
    @Autowired @Qualifier("getGitHubClient") private WebClient client;
    ...
    public Repository getRep(@PathVariable("name") String name, @PathVariable("reposName") String reposName) {
        return client.get().uri("/repos/{name}/{reposName}", name, reposName).retrieve().onStatus(
